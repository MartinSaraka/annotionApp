    ...
    >
      ...
      >
              ...
              <>
                      ...
                      {currentPage === 2 && (
                        <>
                          ...
                          <Text css={{ fontSize: '16px' }}>
                            The top side bar contains all tools necessary for annotating.
                          </Text>
              </>
            )}
            {currentPage === 3 && (
              <>
                          ...
                          <Text css={{ fontSize: '16px' }}>
                            On the right side, you can see the differenct classes and parameters. After clicking on an annotation, you will see the features of the annotation and AI rules.
                          </Text>
              </>
            )}
        </Box>
        ...
        <Box css={{ display: 'flex', justifyContent: 'space-between' }}>
          <Button css={{ margin: '5px' }} onClick={(e) => { e.stopPropagation(); goToPrevPage(); }} disabled={currentPage === 1}>Previous</Button>
          <Button css={{ margin: '5px' }} onClick={(e) => { e.stopPropagation(); goToNextPage(); }} disabled={currentPage === totalPages}>Next</Button>
        </Box>
        <Button ghost onClick={toggleOnboarding} css={{ marginTop: '20px' }}>Close</Button>
      </Box>
    </div>
...
<Button onClick={toggleOnboarding}>Help</Button>
      ...
      <Box css={{ flexDirection: 'row', gap: '$1' }}>
        <Text variant="lg" css={{ color: '$dark4' }} aria-describedby="image-project">Personal</Text>
        <Text role="separator" variant="lg" css={{ color: '$dark4' }}>/</Text>
        <Text as="h1" variant="lg" css={{ color: '$light' }} aria-describedby="image-name">{data?.filename}</Text>
      </Box>

      <Chip small aria-describedby="image-format">{data?.format}</Chip>
    </Box>
  );
};
